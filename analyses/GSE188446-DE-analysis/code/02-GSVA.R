...
metadata <- metadata %>%
  dplyr::mutate(Treatment = factor(Treatment, levels = c("Vehicle", "TAK", "TDM1", "Combo", "Resistance")))

# Select only the PBTA miRNA matched mRNA samples
...
hallmark_gene_sets <- msigdbr::msigdbr(
  species = "Mus musculus", # Can change this to what species you need
...
ggsave(
  ...
  file.path(plots_dir, "gsva_heatmap.tiff"),
  width = 12,
  height = 8
)
